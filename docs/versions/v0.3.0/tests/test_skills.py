...
def _ctx(sim_robot, plc, locomotion) -> SkillContext:
    return SkillContext(
        robot=sim_robot,
...
def test_load_skill_succeeds_when_preconditions_met(sim_robot, mock_plc, sim_locomotion) -> None:
    skill = LoadWorkpieceSkill(_ctx(sim_robot, mock_plc, sim_locomotion))
...
def test_load_skill_blocked_without_preconditions(sim_robot, mock_plc, sim_locomotion) -> None:
    mock_plc.state[Signal.FIXTURE_FREE] = False
    skill = LoadWorkpieceSkill(_ctx(sim_robot, mock_plc, sim_locomotion))
    assert skill.precondition() is False


def test_orchestrator_ticks_sequence(sim_robot, mock_plc, sim_locomotion) -> None:
    skill = LoadWorkpieceSkill(_ctx(sim_robot, mock_plc, sim_locomotion))
    orch = Orchestrator([skill])
    assert orch.tick_once() is True


def test_orchestrator_stops_on_failed_precondition(sim_robot, mock_plc, sim_locomotion) -> None:
