...
from .util.config import PlcConfig, RobotConfig, StationsConfig
...
def build_orchestrator(
    ...
    ctx = SkillContext(robot=robot, motion=motion, plc=plc, locomotion=locomotion)
...
def main(argv: list[str] | None = None) -> int:
    ...
    locomotion = OnboardLocomotion(stations_cfg.stations, driver)
    ...
    try:
        if driver_name == "sim":
            ...
            sim = PlcSimulator(handshake, plc=plc)
