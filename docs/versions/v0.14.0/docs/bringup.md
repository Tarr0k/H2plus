...
**Gut zu wissen:** Das **H2-Modell ist in `unitree_mujoco` bereits enthalten** (`unitree_robots/h2`), und
MuJoCo ist **CPU-Physik → KEINE NVIDIA/RTX-GPU nötig** (das braucht nur Isaac Sim/GR00T).

**Anforderungen (bescheiden):**
- **Ubuntu 20.04/22.04** (Linux; Windows/WSL nicht unterstützt), normaler x86_64-Multi-Core-PC, ~8–16 GB RAM.
- Beliebige/integrierte GPU für den MuJoCo-Viewer (OpenGL) — **kein CUDA**.
- Im Grunde **Stufe B** (SDK + CycloneDDS 0.10.2, siehe `docs/dev_environment.md`) **+ MuJoCo**.

**Automatisch (empfohlen):** Das Skript **`scripts/setup_ubuntu_twin.sh`** erledigt den kompletten
Stack idempotent (apt-Deps → CycloneDDS 0.10.2 → unitree_sdk2 → unitree_sdk2_python → MuJoCo →
unitree_mujoco → h2plus-venv + Tests):
```bash
bash scripts/setup_ubuntu_twin.sh --yes
# braucht non-interaktives sudo (NOPASSWD) und Internet; verändert /opt, ~/.mujoco, ~/h2_twin
```
Die manuellen Einzelschritte (falls du sie nachvollziehen willst):

**Setup — C++-Variante (von Unitree empfohlen):**
```bash
sudo apt install libyaml-cpp-dev libspdlog-dev libboost-all-dev libglfw3-dev cmake build-essential
# unitree_sdk2 nach /opt/unitree_robotics installieren:
git clone https://github.com/unitreerobotics/unitree_sdk2.git && cd unitree_sdk2
mkdir build && cd build && cmake .. -DCMAKE_INSTALL_PREFIX=/opt/unitree_robotics && sudo make install
# MuJoCo-Release (z. B. mujoco-3.3.6) nach ~/.mujoco entpacken + symlinken:
git clone https://github.com/unitreerobotics/unitree_mujoco.git
cd unitree_mujoco/simulate && ln -s ~/.mujoco/mujoco-3.3.6 mujoco
mkdir build && cd build && cmake .. && make
./unitree_mujoco -r h2 -s scene_terrain.xml       # H2 im Simulator laden
```

**Setup — Python-Variante (einfacher):**
```bash
pip3 install -e unitree_sdk2_python      # braucht CycloneDDS 0.10.2 (CYCLONEDDS_HOME setzen)
pip3 install mujoco pygame
# simulate_python/config.py auf ROBOT = "h2" stellen, dann:
python3 unitree_mujoco/simulate_python/unitree_mujoco.py
```

**Dann unseren Stack gegen den Twin fahren** (2. Terminal, im h2plus-Repo):
```bash
python -m h2_loader.bringup --iface lo --phase 1        # DDS/LowState aus der Sim lesen
python -m h2_loader.bringup --iface lo --phase 2        # Loco-FSM (Damp->Start->StandUp) in der Sim
# volle Lade-Logik gegen die Physik: robot.sim_mujoco.yaml verwenden (driver sdk, lo, domain 1)
```
